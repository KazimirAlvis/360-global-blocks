...
= 1.3.73 =
* Reformatted Two Column Text editor selectors for readability/consistency without changing behavior.

= 1.3.72 =
* Refined Two Column Text spacing: removed forced paragraph resets, set paragraph top margin to 0, and updated list spacing/indent controls in both editor and frontend styles.
...
* Fixed the Full Page Hero editor UI so the image upload + RichText fields are clickable/visible.

= 1.3.69 =
* Removed legacy Test Hero and Symptoms AI blocks.
* Bundled all 360 Global Blocks frontend CSS into a single site-wide stylesheet to reduce requests.

= 1.3.68 =
* Prevent duplicate block CSS/JS enqueues when assets are already declared in `block.json`.

= 1.3.67 =
* Added the Annotated Bibliography (Simple) block with a single RichText bibliography field, auto-linking for pasted URLs, and centered 1140px layout styling.

= 1.3.66 =
* Added Comparison Table controls for the header row font size and header row background color.

= 1.3.65 =
* Reduced Two Column Text heading, paragraph, and list spacing so headings sit closer to content and list items no longer create excessive vertical gaps.
...
= 1.3.62 =
* Restore Two Column body content on the frontend by serializing the editor InnerBlocks and prioritizing migrated markup inside the PHP render callback.

= 1.3.61 =
* Preserve legacy Two Column body markup when migrating to InnerBlocks by defaulting to a Core HTML block whenever raw HTML is detected, so bespoke wrappers (lists, layout divs, classes) stay intact.
* Allow Core HTML blocks in the editor body area to keep custom structured content editable instead of stripping it during migration.
...
* Added Largest Contentful Paint hints (`loading="eager"`, `fetchpriority="high"`, `decoding="async"`) and a mobile-first `sizes` attribute to prioritise the hero image while keeping it responsive.
